...
## Document de référence — Version 4.5

**Date** : 29 mars 2026
**Statut** : Phases 1, 2 et 3 implémentées et validées
...
- WiFi AP+STA simultané : AP `192.168.5.1` (accès local, WPA2), STA `192.168.4.10` (vers LilyGo, ouvert)
- Framework : Arduino (PlatformIO), SDK v4.4.4, Arduino core 10812
- Architecture non-bloquante, orchestrée par TaskManager (callbacks < 50ms)
- RTC DS3231 intégrée (horodatage fiable dès le boot)
- Power save WiFi désactivé (`WiFi.setSleep(false)`) — carte sur secteur

**LilyGo T-SIM7080G-S3** (pont WiFi-GSM)
- Rôle : pont NAT WiFi → Cat-M (PPPoS) vers internet + service SMS + communication UDP
- WiFi SoftAP `192.168.4.1`, SSID `Pont_Wifi-GSM_de_la_serre`, 1 client max, **WiFi ouvert**
- Modem SIM7080G, APN `domotec82.fr`
- PMU AXP2101 (batterie intégrée)
- Framework : ESP-IDF pur (FreeRTOS), WiFi firmware `57982fe`
...
Internet (Cat-M)
      |
  [ LilyGo 192.168.4.1 ]  SoftAP ouvert + NAT/NAPT
      |                     (WIFI_AUTH_OPEN)
  [ Waveshare 192.168.4.10 ]  STA (ouvert) + AP 192.168.5.1 (WPA2)
      |
  [ Téléphone / PC ]
```

**Sécurité du lien Waveshare ↔ LilyGo :**
Le lien WiFi entre les deux cartes est ouvert (pas de WPA2). Ce choix est
justifié par le contexte : réseau privé dédié, 1 seul client, serre isolée.
Le MQTT est protégé par TLS de bout en bout (ISRG Root X1) indépendamment
du WiFi. Le seul contenu non chiffré sur le lien local est le protocole UDP
(heartbeats, STATE, ordres SMS) — sans donnée sensible.
L'AP de la Waveshare (`Serre_de_Marie-Pierre`) conserve son mot de passe
WPA2 car il donne accès au serveur web local.
...
3. GPIO modem
4. PPPoS → attente IP
5. WiFi SoftAP (`WIFI_AUTH_OPEN`, pas de mot de passe)
6. NAPT (DNS + NAT)
7. Mutex modem
...
1. Boot + WiFi AP (~9s)
2. WiFi STA connecté à LilyGo (~10s, quasi-instantané en WiFi ouvert)
3. MQTT connecté au broker (~32s, TLS ~3s)
4. NTP synchro (~133s, délai 2 min après WiFi, réussit au 1er essai)
