...
## Document de référence pour implémentation — Version 4.2

**Date** : 26 mars 2026
**Statut** : Validé — Phase 1 partiellement implémentée
**Révision** : v4.2 — MqttManager réactif (callback DataLogger), démarrage conditionnel WiFi STA, nommage explicite
...
**Note** : quand BridgeManager sera implémenté, la condition dans
`ensureMqttStarted()` pourra être enrichie (ex: `WiFiManager::isSTAConnected()
&& BridgeManager::isModemReady()`). Le design actuel le permet sans
refactoring.

~~**Ancienne machine d'états (v4.1)**~~
La v4.1 décrivait une machine d'états complète (IDLE, CHECK_MODEM,
CONNECTING, CONNECTED, PAUSED, WAIT_MODEM, WAIT_RETRY) avec un `handle()`
périodique. Ce design n'a pas été retenu : MqttManager est purement réactif
et esp_mqtt gère sa propre reconnexion. Si BridgeManager introduit des
besoins de coordination plus complexes (PAUSED pour SMS, WAIT_MODEM),
une machine d'états pourra être ajoutée à ce moment-là.
...
### 6.4 Intégration dans main.cpp (implémenté)
...
// Callback DataLogger → publication MQTT automatique
...
~~**Ancienne stratégie (v4.1)** : publication périodique via tâches
TaskManager dédiées (`publishSensors()` toutes les heures,
`publishSystem()` toutes les 5 minutes) + publication immédiate
pour les événements critiques. Ce modèle hybride n'a pas été retenu
car le callback `onDataPushed()` couvre tous les cas de manière
...
**Defines supprimés (v4.2)** : `MQTT_HANDLE_PERIOD_MS`,
...
d'états et aux tâches périodiques, non utilisés dans le design réactif.
Le keepalive et la reconnexion sont gérés par esp_mqtt en interne.
...
3. Ajouter les defines dans NetworkConfig.h et TimingConfig.h **✅ FAIT**
4. Intégrer dans main.cpp (init + callback DataLogger) **✅ FAIT**
...
4. Heartbeat régulé (à intégrer quand BridgeManager sera implémenté)
...
- **MqttManager réactif** (§6.2) : plus de machine d'états, plus de
  `handle()`. MqttManager est déclenché par le callback `onDataPushed()`
  de DataLogger. esp_mqtt gère sa tâche FreeRTOS pour la connexion TLS,
  le keepalive et la reconnexion automatique.
- **Démarrage conditionnel WiFi STA** (§6.2) : `init()` configure le
  client esp_mqtt mais ne le démarre pas. `ensureMqttStarted()` fait
  le `esp_mqtt_client_start()` au premier push après connexion WiFi STA.
  MqttManager est totalement autonome, aucune intervention de main.cpp.
- **Aucune tâche TaskManager** (§6.4) : suppression des 3 tâches prévues
  (`handle`, `publishSensors`, `publishSystem`). Le rythme de publication
  est dicté par les capteurs et le DataLogger, pas par MqttManager.
- **Publication immédiate** (§6.5) : chaque `DataLogger::push()` publie
  immédiatement si MQTT est connecté. Remplace le modèle hybride
  (périodique + immédiat) prévu initialement.
- **Nommage explicite** : `connected` → `mqttConnected`, `isConnected()`
  → `isMqttConnected()`, ajout `mqttStarted` et `ensureMqttStarted()`.
- **Defines simplifiés** (§6.7) : suppression des 8 defines liés à
  l'ancienne machine d'états et aux tâches périodiques.
- **Phase 4 (commandes)** reste inchangée dans le plan mais devra
  prévoir un mécanisme de dépilage pour les messages reçus sur
  `serre/cmd/#` (§6.3).