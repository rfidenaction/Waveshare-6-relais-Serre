...
void WiFiManager::handle()
{
    ...
    case State::AP_START: {
        ...
        //
        // ── Canal WiFi ──────────────────────────────────────────────
        // En mode AP+STA, l'ESP32 n'a qu'UNE seule radio : l'AP et
        // le STA DOIVENT être sur le même canal. La LilyGo (pont
        // WiFi/GSM) force son AP sur ce canal. On doit s'aligner.
        //
        // Canaux non-chevauchants en 2.4 GHz : 1, 6 ou 11
        // Éviter les canaux 12-14 (problèmes connus sur ESP32)
        //
        // Pour changer : modifier la valeur ci-dessous ET le
        // define WIFI_AP_CHANNEL dans le main.cpp de la LilyGo.
        // ─────────────────────────────────────────────────────────────
        static constexpr int WIFI_CHANNEL = 1;  // Doit correspondre à la LilyGo

        unsigned long t0 = millis();
        bool ok = WiFi.softAP(WIFI_AP_SSID, WIFI_AP_PASSWORD, WIFI_CHANNEL);
    ...
    case State::STA_BEGIN: {
        ...
        if (firstAttempt) {
            ...
            if (n <= 0) {
                Console::warn("WiFi", "Scan boot : aucun réseau trouvé");
            } else {
                Console::info("WiFi", "Scan boot : " + String(n) + " réseau(x) trouvé(s)");
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    Console::info("WiFi", info);

                    if (WiFi.SSID(i) == WIFI_STA_SSID) {
                        targetFound = true;
                    }
                }
                if (targetFound) {
                    Console::info("WiFi", "Réseau cible '" + String(WIFI_STA_SSID) + "' TROUVÉ");
                } else {
    ...
    case State::STA_CONNECTING: {
        ...
        // ── Détection d'échec rapide ────────────────────────────────
        // Quand le driver reçoit un refus (ex: "Association refused
        ...
        if (now - connectStartMs >= STA_EARLY_FAIL_GRACE_MS) {
            if (status == WL_DISCONNECTED || status == WL_CONNECT_FAILED) {
                Console::info("WiFi", "STA échec rapide détecté après "
                             + String((now - connectStartMs) / 1000) + "s"
                             + ", status=" + wlStatusToString(status));
                staConnected = false;
        ...
        // Log périodique (diagnostic sans spam)
        ...
        // Timeout (filet de sécurité, rarement atteint)
